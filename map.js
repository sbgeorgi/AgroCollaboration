      ...
      .map(
        (item, idx) => `<div class="carousel-slide">
            ...
            ? `<video src="${escapeHtml(
              item.url
            )}" controls autoplay muted loop playsinline alt="Location video ${idx + 1
            }"></video>`
            : `<img src="${escapeHtml(
              item.url
  ...
  return `
    ...
    point.name
        ...
        .map(
          (c) => `
                        <div class="flex items-center gap-2 bg-slate-100 rounded-full pr-3 py-1 text-sm cursor-pointer hover:bg-slate-200 transition-colors" data-open-profile="${c.id}">
                            <img class="w-6 h-6 rounded-full object-cover" src="${c.public_avatar_url ||
      ...
      : ''
                ...
                ${isAdmin
      ? `<button class="flex-1 btn btn-secondary btn-edit" data-id="${point.id
      }"><i data-lucide="edit-3" class="w-4 h-4"></i>Edit</button>
                       <button class="flex-1 btn btn-danger btn-delete" data-id="${point.id
    ...
    resultsContainer.innerHTML = filteredUsers
      .map(
        (user) => `<div class="user-search-item" data-user-id="${user.id}">
              <img src="${user.public_avatar_url || 'static/avatar_placeholder.png'
...
function wireMapUI() {
    ...
    $('#projects-table tbody').addEventListener('click', (e) => {
        ...
        // Fix: Use String conversion to ensure ID matching works (uuid vs string)
        const point = state.mapPoints.find((p) => String(p.id) === String(pointId));
        if (point) openViewPanel(point);
    });
    $('#closeViewPanelBtn').addEventListener('click', closeViewPanel);
    
    // FIX: Fetch full profile data on click so the modal isn't empty
    document.addEventListener('click', async (e) => {
        const btn = e.target.closest('[data-open-profile]');
        if (btn) {
            e.preventDefault();
            e.stopPropagation(); // Prevent map interactions while clicking profile
            ...
            if (profileId) {
                ...
                // 2. Open modal with full data (or fallback to partial if fetch fails)
                if (profile) {
                    openProfileModal(profile);
                } else if (state.selectedPoint && state.selectedPoint.collaborators) {
