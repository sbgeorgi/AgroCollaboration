...
export function initEventLogic(deps) {
  ...
  async function renderDescriptionTab() {
    ...
    // Use end_time if available, otherwise fallback to start_time
    const baseTime = ev.end_time ? new Date(ev.end_time) : new Date(ev.start_time);
    
    // Add a 4-hour buffer (4 * 60 * 60 * 1000 milliseconds) to keep the link alive during & after the event
